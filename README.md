...
后台管理页面：[https://vue2.clock-main.dev](http://139.9.82.205:8090/)
...
``` bash
         ...
         ───────────────────────────────└───────────────────────────────
        |                               |                               |
    添加待办                      用户获取打卡记录                用户获取自习记录
        |         ├─判断打卡状态         |                               |
选择待办开始打卡───├────────────────>添加打卡记录———————————>(当天首次)添加自习记录,status设为当前待办id
        |         ├─更新连续打卡天数      |                              |       (-1为未打卡状态))
        |                               ├─设置本次打卡时长及结束时间      ├─更新今日自习时长及今日完成待办
        |                               ├─时长少于一分钟删除记录          ├─设置status=-1
结束此次打卡—————————————————————————————|———————————————————————————————├─更新用户打卡总时长
```
